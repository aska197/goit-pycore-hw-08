...
def main():
    book = load_data() # Load data after running the program
    ...
    while True:
        ...
        if command in ['close', 'exit']:
            print('Good bye!')
            save_data(book) # Save data before finishing the program
            break
            ...
            if args and args[0] == 'birthday':
                add_birthday(args[1:], book)
            else:
                handle_add_command(args, book)

        elif command == 'change phone':
            handle_change_phone_command(args, book)

        elif command == 'phone':
            handle_phone_command(args, book)

        elif command == 'all':
            handle_all_command(book)
